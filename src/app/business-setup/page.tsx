  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const businessData = {
        ...
        businessAverageDailyCustomerCount: parseInt(
          formData.averageDailyCustomerCount,
        ),
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        businessSubType: e.target.value,
                      })
                    }
                    required
                  >
              ...
              {(formData.latitude !== 0 || formData.longitude !== 0) && (
                <div
                  className={`rounded-lg p-3 ${
                    formData.latitude === 37.5665 &&
                    formData.longitude === 126.978
                ...
                >
                  ...
                  >
                    🗺️ 좌표 정보: 위도 {formData.latitude.toFixed(6)}, 경도{" "}
                    {formData.longitude.toFixed(6)}
                    {formData.latitude === 37.5665 &&
                      formData.longitude === 126.978 && (
                        <span className="mt-1 block text-yellow-700">
                          ⚠️ 기본 좌표가 사용되었습니다. 카카오 개발자 콘솔에서
                          &apos;로컬(지도/로컬)&apos; 서비스를 활성화하면 정확한
                          좌표를 받을 수 있습니다.
                        </span>
                      )}
                  </p>
            ...
            <div className="space-y-4">
              ...
              <Input
                label="평균 일간 고객 수"
                type="number"
                value={formData.averageDailyCustomerCount}
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    averageDailyCustomerCount: e.target.value,
                  })
                }
                placeholder="100"
                helperText="명 단위로 입력하세요"
                required
              />
              ...
              <Input
                label={
                  formData.businessType === BusinessType.ACCOMMODATION
                    ? "객실 수"
                    : "좌석 수"
                }
                type="number"
