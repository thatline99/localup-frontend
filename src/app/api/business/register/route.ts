...
interface BusinessData {
  ...
  businessType: string;
  businessItem: string;
  businessAverageDailyCustomerCount: number;
...
async function registerOrUpdateBusiness(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      error.message ||
        `사업정보 ${method === "PATCH" ? "업데이트" : "등록"}에 실패했습니다.`,
    );
  }
...
export async function POST(request: NextRequest) {
  try {
    const {
      ...
      businessType,
      businessItem,
      businessAverageDailyCustomerCount,
    ...
    const business = await registerOrUpdateBusiness(
      businessData,
