  ...
  try {
    ...
    if (!responseText || responseText.trim() === "") {
      ...
      if (setCookieHeader && setCookieHeader.includes("accessToken=")) {
        ...
        const backendAccessToken = accessTokenMatch
          ? accessTokenMatch[1]
          : null;
...
async function authKakaoUser(kakaoProfile: {
  ...
  try {
    ...
    const loginResponse = await fetch(
      `${process.env.BACKEND_API_URL}/auth/kakao/check`,
      {
        method: "POST",
        headers: {
    ...
    if (loginResponse.status === 404 || loginResponse.status === 401) {
      const signupResponse = await fetch(
        `${process.env.BACKEND_API_URL}/auth/kakao/signup`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
        ...
        if (!backendAccessToken && setCookieHeaders.length > 0) {
          for (const cookieHeader of setCookieHeaders) {
            if (cookieHeader.includes("accessToken=")) {
              ...
              backendAccessToken = accessTokenMatch
                ? accessTokenMatch[1]
                : null;
              break;
            }
    ...
    // 기타 로그인 오류
    console.error(
      `❌ 카카오 로그인 실패 - 예상치 못한 응답: ${loginResponse.status}`,
    );
        ...
        if (account?.provider === "kakao") {
          ...
          if ((user as { backendAccessToken?: string }).backendAccessToken) {
            token.backendAccessToken = (
              user as { backendAccessToken?: string }
            ).backendAccessToken;
          } else {
          }
    ...
    async session({ session, token }) {
      ...
      if (token) {
        ...
        if (token.isNewUser !== undefined) {
          (session.user as { isNewUser?: boolean }).isNewUser =
            token.isNewUser as boolean;
        }
      }
      return session;
    },
